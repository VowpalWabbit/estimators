...
def test_estimate_10_from_10s():
    ...
    assert_interval_covers(
        lambda: clopper_pearson.Interval(rmin=0, rmax=10), simulator, 10
    )
...
def test_estimate_negative_constant():
    ...
    assert_interval_covers(gaussian.Interval, simulator, -1)
    assert_interval_covers(
        lambda: clopper_pearson.Interval(rmin=-1, rmax=0), simulator, -1
    )
...
def test_simple_convergence():
    ...
    assert_interval_within(
        lambda: clopper_pearson.Interval(0, r), lambda: simulator(r), expected
    )
    assert_interval_within(
        lambda: cressieread.Interval(rmin=0, rmax=r), lambda: simulator(r), expected
    )
    ...
    assert_interval_within(
        lambda: cs.Interval(rmin=0, rmax=r), lambda: simulator(r), expected
    )
...
def test_zero_variance_reward():
    ...
    def simulator():
        for i in range(100):
            yield {"p_log": 0.5, "r": r, "p_pred": 0.5}
    ...
    assert_interval_within(
        lambda: clopper_pearson.Interval(0, r), lambda: simulator(), expected
    )
    assert_interval_within(
        lambda: cressieread.Interval(rmin=0, rmax=r), lambda: simulator(), expected
    )
    assert_interval_within(
        lambda: cs.Interval(rmin=0, rmax=r), lambda: simulator(), expected
    )
...
def test_convergence_with_p_drop():
    ...
    def simulator():
        for _ in range(1000):
            yield {"p_log": 1, "r": 1, "p_pred": 1, "p_drop": 0}
        for _ in range(500):
            yield {"p_log": 1, "r": 0, "p_pred": 1, "p_drop": 0.5}
...
def test_cats_ips():
    ...
    max_value = 32
    bandwidth = 1
    cats_transformer = cats_utils.CatsTransformer(
        num_actions=8, min_value=0, max_value=max_value, bandwidth=bandwidth
    )
    ...
    for logged_action, r, logged_prob in zip(action_logs, rewards, prob_logs):
        ...
        if logged_action < (max_value / 2.0):
            ...
            data = cats_transformer.transform(
                data, pred_action
            )  # pred_action should be too far away, so pred_p should be 0
            assert data["pred_p"] == 0.0
        else:
            pred_action = logged_action
            data = cats_transformer.transform(
                data, logged_action
            )  # same action, so pred_p should be 1
